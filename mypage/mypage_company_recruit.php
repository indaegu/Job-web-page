...
<head>
  ...
  <!-- 이름 가져오기 -->
    ...
    <?php
      ...
      $sql_s = mq("SELECT DISTINCT * FROM user INNER JOIN user_recruitment_mapping ON user.아이디 = user_recruitment_mapping.아이디;");
      while ($board = $sql->fetch_array()) {
        if($board['채용정보ID'] == $_GET['채용정보ID'])
        { ?>
    ...
    <div id = "board_area">
      <table class = "list-table">
        ...
        <?php
          while ($board_s = $sql_s->fetch_array())
          {
            if($board_s['채용정보ID'] == $_GET['채용정보ID'])
            { ?>
            <tbody>
              <tr>            
                <td width = 100> <?php echo $board_s["이름"]; ?> </td>
                <td width = 50> <?php echo $board_s["성별"]; ?> </td>
                <td width = 100> <?php echo $board_s["생년월일"]; ?> </td>
                <td width = 100> <a href="mypage_company_apply.php?채용정보ID=<?php echo $board_s['채용정보ID']; ?>&아이디=<?php echo $board_s['아이디']; ?>" onclick=" return confirm('채용하시겠습니까?');">[채용하기]</a></td>
              </tr>
            </tbody>
          <?php }} ?>
      </table>
